...
public class Plank extends Entity {
    ...
    public Plank(float x, float y) {
        super(x, y);
        plankFrames = AnimHelper.getFrames(GameJam.assets.manager.get(Assets.plank),1,3);
        ...
        //Collision boundary
        bounds = new Rectangle(pos.x,pos.y,plankFrames[0].getRegionWidth()-48,plankFrames[0].getRegionHeight());
    ...
    public void draw(SpriteBatch batch) {
        batch.draw(plankFrames[texture_index],pos.x,pos.y,64,16);

        //Draw hammer when hitting
        batch.draw(currentHammerFrame,clickPos.x,clickPos.y);

        if(enableText){
            font.draw(batch,"w0w!",pos.x+8,pos.y+36);
        }
    }
    ...
    public void update(float delta) {
        ...
        if(Gdx.input.isButtonJustPressed(Input.Buttons.LEFT) && bounds.contains(boundClickPos.x,boundClickPos.y)){
            ...
            if(texture_index!=2 && times_hammered>8){
                texture_index++;
            }
        }
